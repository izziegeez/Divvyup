...
export default class register extends Component {
  ...
  handleResponse = res => {
    let newState = res.data ? { redirect: true, user: res.data.user }
  ...
  validPassword = password => {
    ...
    let needLowercase = password.toUpperCase() === password,
        ...
        needLonger = password.length <= 8,
        testPassed = !(needLowercase || needUppercase || needSpecial || needLonger);
    return { testPassed, needLowercase, needUppercase, needSpecial, needLonger };
  }

  rejectionCondition = formInfo => {
    const { testPassed, needLowercase, needUppercase, needSpecial, needLonger } = this.validPassword(formInfo.password);
    if (!testPassed) {
      let errorMessages = ['Password must:\n',
          needLonger ? 'Be 8 Characters Long\n' : '',
      ...
      let message = errorMessages.reduce((accumulator, curr) => accumulator + curr),
  ...
  render() {
    ...
    return (
      <div className='container'>
          ...
          <AccountForm submitRoute='/register' formName='Create an Account' handleResponse={this.handleResponse}
