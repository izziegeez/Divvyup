...
const createParty = async (name, cost, ownerID) => {
    let allParties = await Party.find({ archived: false }),
        ...
        count = 0;
    while (allPartyIDs.find((partyID => partyID === id)) && count < 37657312) {  // while a match is found
        id = generateRandIdentifier();
        count++;
    }
...
module.exports = {
    ...
    deleteData: async () => {
        await User.deleteMany({}).then(console.log('Removed all User documents'.yellow)).catch(err => console.error('Error removing User documents'.red, err));
        await Party.deleteMany({}).then(console.log('Removed all Party documents'.yellow)).catch(err => console.error('Error removing Party documents'.red, err));
    }
